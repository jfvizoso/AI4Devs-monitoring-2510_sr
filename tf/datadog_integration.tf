# Integración AWS-Datadog según documentación oficial
# https://docs.datadoghq.com/integrations/guide/aws-terraform-setup/
# Para sitio EU1 (datadoghq.eu) el account ID de Datadog es 464622532012

data "aws_caller_identity" "current" {}

locals {
  datadog_aws_account_id = "464622532012" # Datadog EU site
}
...
locals {
  ...
  chunk_assignments = [for cumulative_size in local.cumulative_sizes : floor(cumulative_size / local.target_chunk_size)]
...
# Crear primero la integración en Datadog para obtener el external_id
...
locals {
  datadog_external_id = datadog_integration_aws_account.datadog_integration.auth_config.aws_auth_config_role.external_id
}
...
resource "aws_iam_role" "datadog_aws_integration" {
  ...
  description        = "Role for Datadog AWS Integration (EU1)"
